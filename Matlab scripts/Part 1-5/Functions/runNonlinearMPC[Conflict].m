...
for i = 1:NonLinMPC.Length
    [solutions,diagnostics] = NonLinMPC.controller{x};
    if diagnostics == 1
        error('Infeasible!');
    end
    NonLinMPC.U = solutions;                                    % Input prediction horizon
    NonLinMPC.X = NonLinMPC.A*x+NonLinMPC.B*NonLinMPC.U;        % State prediction horizon
    ...
    NonLinMPC.Xtrajectory(i,:) = NonLinMPC.X(:,1);
    ...
    if (i == 1)
        ...
        x1_sim = NonLinMPC.x1simulink(i-1);
        x2_sim = NonLinMPC.x2simulink(i-1);
    end
    ...
    % Save results
    NonLinMPC.x1simulink(i) = x1simu(end);
    NonLinMPC.x2simulink(i) = x2simu(end);

    x = [x1simu(end) x2simu(end)]';
    x = NonLinMPC.A*x + NonLinMPC.B*NonLinMPC.U(1);

    
    if (SimSettings.NonLinMPCplot)
        pause(0.025)        % Add a pause for plotting
    end
end
